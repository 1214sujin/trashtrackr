	...
	if (topic=='TrashTrackr/RP') {
		...
		if (msg.load_img) {	// 적재량이 90% 이상인 경우
			...
			try {
				...
				fs.writeFileSync(__dirname+'/../public/images'+msg.load_img_name, imageBuffer)
		...
		if (msg.fire_img) {
			...
			try {
				...
				fs.writeFileSync(__dirname+'/../public/images'+msg.fire_img_name, imageBuffer)
		...
		try {
			...
			fs.writeFileSync(__dirname+'/../public/images/monitor.jpg', imageBuffer)
			ee.emit('photo')
		} catch (error) {
			console.error('Error:', error.message)
		}
