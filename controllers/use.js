...
module.exports = {
	...
	gu: (req, res) => {
		...
		sql0 = "select dong, dong_id, count(rp_time)/10 as `use`\
				from (select rp_time, pos from replacement r join bin b on r.bin_id=b.bin_id where ?<=date_format(rp_time, '%Y-%m')<=?) as s\
	...
	dong: (req, res) => {
		var { dong_id } = req.params
		var { start, end } = req.query
		sql0 = "select b.bin_id, count(rp_time) as `use` from (select bin_id, rp_time from replacement where ?<=date_format(rp_time, '%Y-%m')<=?) as r\
				right join bin b on r.bin_id=b.bin_id where pos=? group by bin_id order by `use` desc limit 5;"
		sql1 = "select b.bin_id, count(rp_time) as `use` from (select bin_id, rp_time from replacement where ?<=date_format(rp_time, '%Y-%m')<=?) as r\
	...
	dong_detail: (req, res) => {
		...
		var sql0 = `select period, count(period) as 'use'
					from ( select bin_id, rp_time, date_format(rp_time, '%Y-%m') as period from replacement where ?<=date_format(rp_time, '%Y-%m')<=? ) as r
